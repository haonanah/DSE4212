...
class PolicyGradient:
    ...
    def train(self, episodes=100, callback=None, validation_freq=1, patience=10, threshold=0.01):
        ...
        Args:
            ...
            threshold: Minimum relative improvement required to reset patience.
        """
        best_validation_performance = None  # Initialize as None
        ...
        for episode in tqdm(range(1, episodes + 1)):
            ...
            while not done:
                ...
                if callback is not None:
                    ...
                    if not continue_training:
                        print(f"Early stopping triggered by callback at episode {episode}, step {step_counter}")
                        return  

                obs = next_obs

            # Perform gradient ascent after each episode
            self._gradient_ascent()
            ...
            if self.validation_env and episode % validation_freq == 0:
                ...
                # Check if validation_performance is finite
                if not np.isfinite(validation_performance):
                    print(f"Validation performance is not finite: {validation_performance}. Skipping this validation.")
                    continue
    ...
    def evaluate_on_validation(self):
        ...
        # Alternatively, you can calculate cumulative return
        # cumulative_return = np.prod([1 + r for r in rewards]) - 1

        return sharpe_ratio  # or return cumulative_return
